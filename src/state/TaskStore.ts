...
export class TaskStore {
    ...
    public updatingTask(listId: string, task: Task): TaskStore {
        ...
        if (this.compare(tasks.get(task.id)!, task) === 0) {
            makeImmutable(task);
    ...
    public addingTask(listId: string, task: Task): TaskStore {
        ...
        let index = 0;
        if (taskIds.size > 0) {
            if (this.compare(tasks.get(taskIds.last(undefined)!)!, task) < 0) {
                index = taskIds.size;
            } else if (this.compare(tasks.get(taskIds.first(undefined)!)!, task) <= 0) {
                let start = 0;
    ...
    public deletingTask(listId: string, taskId: string): TaskStore {
        ...
        while (start <= end) {
            index = Math.floor((start + end) / 2);
            const compareResult = this.compare(tasks.get(taskIds.get(index)!)!, task);
        ...
        return new TaskStore(
            ...
            this.taskIds.set(listId, taskIds.delete(Math.floor((start + end) / 2))),
            this.comparator
        );
    }
}